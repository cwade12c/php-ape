...
try {
    ...
    $sql = $db->prepare(
        "insert into exam(quarter, date, location, state, passing_grade, duration, start_time, cutoff)
    ...
    $id = $db->lastInsertId();
    $sql = $db->prepare(
        "insert into in_class_exams (exam_id, teacher_id) values ('$id', 3586832)"
    );
    $sql->execute();
    ...
    if (isset($_POST["category1"])) {
        ...
        $sql = $db->prepare(
            "insert into exam_category(category_id, exam_id, possible) 
						values('$cat1', '$id', '$points1')"
        );
        $sql->execute();
        $tempId = $db->lastInsertId();
        $sql = $db->prepare(
            "insert into assigned_graders(exam_cat_id, grader_id)
