...
try {
    ...
    $stmt = $conn->prepare(
        "SELECT exam_id FROM exam_roster WHERE student_id=" . $idStudent . ""
    );
    $stmt->execute();
    ...
    while ($row = $stmt->fetch()) {
        ...
        $examStat = $conn->prepare(
            "SELECT id, quarter, date, location, state, passing_grade, duration FROM exam WHERE id='"
            . $examID . "'"
        );
        $examStat->execute();
        ...
        $locstat = $conn->prepare(
            "SELECT name FROM location WHERE id='" . $exam['location'] . "'"
        );
        $locstat->execute();
        $location = $locstat->setFetchMode(PDO::FETCH_ASSOC);
        $loc = $locstat->fetch()['name'];

        $quartstat = $conn->prepare(
            "SELECT name FROM quarter WHERE id='" . $exam['quarter'] . "'"
        );
        $quartstat->execute();
